...
export const getRegularFont = cache(async () => {
  const response = await readFile(`${rootdir}/public/static/assets/fonts/NotoSansTC-Regular.ttf`);
...
export const getBoldFont = cache(async () => {
  const response = await readFile(`${rootdir}/public/static/assets/fonts/NotoSansTC-Bold.ttf`);
  const font = Uint8Array.from(response).buffer;
  return font;
});
