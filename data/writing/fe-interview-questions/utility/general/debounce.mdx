...
1. 這個 throttle 的實作方式非常相近，但較於 throttle 是在 timeout 結束時將開關再次打開， debounce 則是在 timeout 結束時呼叫 `func` 並將 `timer` 設為 `null`。
2. 造著這個思路，我們可以先宣告一個 `timer` 變數，並在 debounce 函式中返回一個新的函式。
