---
title: '前端面試手寫練習 - lodash _.throttle'
date: '2024-06-12'
tags: ['javascript']
draft: false
summary: lodash 函式庫中的 _.throttle 函式
topic: '基本題型'
---

## 問題

throttle 是一個函式，它可以限制一個函式在一段時間內只能執行一次。當函式被調用時，它將會在 `wait` 毫秒後執行。如果在這段時間內函式再次被調用，將會被忽略。

可以把它想像成節流的概念，可以限制某些事件或是行為其被觸發的頻率。

```typescript
function throttle(func: Function, wait: number): Function
```

### 示意圖

```bash
加入 throttle 之前:
─ A ─ B ─ C ─ ─ D ─ ─ ─ ─ ─ ─ E ─ ─ F ─ G

加入 throttle 之後:
─ A ─ ─ ─ C ─ ─ ─D ─ ─ ─ ─ ─  E ─ ─ ─ ─ G
```

可以看到加入 throttle 之後，函式 `B` 與 `F` 被忽略了，而在忽略的那段時間就是冷卻時間。

## 範例

舉例來說，有時需要根據頁面的寬度來切換手機或是桌機的版面，我們可以透過透過 `resize` 事件來監聽視窗大小的變化，然而這不需要在每一次 `resize` 事件觸發時都去執行，這時候就可以透過 throttle 來限制執行的頻率。

```javascript
window.addEventListener('resize', throttle(onResize, 1000))
```

## 練習區

在了解問題後，可以嘗試先寫下你的思路，再到下方的練習區域實際寫出程式碼。

<Sandpack lang="test-ts" activeFile="index.js" showTests={true}>

```js file=index.js
/**
 * @param {Function} func
 * @param {number} wait
 * @returns {Function}
 */
function throttle(func, wait) {
  // your code here
}

export { throttle }
```

```js file=throttle.test.js hidden=true
import { throttle } from './index.js'

describe('throttle', () => {
  test('should be a function when initialized', () => {
    const throttledFunc = throttle(() => {}, 50)
    expect(throttledFunc).toBeInstanceOf(Function)
  })

  test('should invoke callback immediately upon first call', () => {
    let count = 0
    const throttledIncrement = throttle(() => {
      count++
    }, 50)

    expect(count).toBe(0)
    throttledIncrement()
    expect(count).toBe(1)
  })

  test('should throttle subsequent immediate invocations', () => {
    let count = 0
    const throttledIncrement = throttle(() => {
      count++
    }, 50)

    expect(count).toBe(0)
    throttledIncrement()
    expect(count).toBe(1)
    throttledIncrement()
    expect(count).toBe(1)
  })

  test('should throttle invocations with delay', (done) => {
    let count = 0
    const throttledIncrement = throttle(() => {
      count++
    }, 100)

    expect(count).toBe(0)
    throttledIncrement()
    expect(count).toBe(1)

    setTimeout(() => {
      throttledIncrement()
      expect(count).toBe(1)
    }, 25)

    setTimeout(() => {
      throttledIncrement()
      expect(count).toBe(1)
      done()
    }, 50)
  })

  test('should allow invocation after throttle period', (done) => {
    let count = 0
    const throttledIncrement = throttle(() => {
      count++
    }, 50)

    expect(count).toBe(0)
    throttledIncrement()
    expect(count).toBe(1)

    setTimeout(() => {
      throttledIncrement()
      expect(count).toBe(2)
      done()
    }, 60)
  })

  test.skip('[bonus] throttle function maintains context and arguments', () => {
    const mockFunc = jest.fn()
    const context = { value: 42 }
    const throttledFunc = throttle(function () {
      return mockFunc(this.value)
    }, 1000)

    // Call the function with a specific context
    throttledFunc.call(context)
    expect(mockFunc).toHaveBeenCalledTimes(1)
    expect(mockFunc).toHaveBeenCalledWith(42)

    // Call the function within the throttle period with different context
    const newContext = { value: 99 }
    throttledFunc.call(newContext)
    expect(mockFunc).toHaveBeenCalledTimes(1) // Still 1 because of throttling

    // Fast forward time by 1000ms
    jest.advanceTimersByTime(1000)

    // After the throttle period, the second call should execute
    expect(mockFunc).toHaveBeenCalledTimes(2)
    expect(mockFunc).toHaveBeenCalledWith(99)
  })
})
```

</Sandpack>

## 筆者思路

1. 由於這是節流函式，所以需要有了開關來控制是否可以執行。所以先建立一個變數 `isThrottled` 來控制是否可以執行。
2. 接下來就是確認當每次函式被調用時，是否可以執行。如果 `isThrottled` 為 `true`，則直接返回。
3. 如果可以執行，則將 `isThrottled` 設為 `true`，並可以夠過 setTimeout 在 `wait` 毫秒後將 `isThrottled` 設為 `false`。

## 筆者解答

```javascript
function throttle(func, wait) {
  let isThrottled = false

  return function (...args) {
    if (isThrottled) {
      return
    }

    isThrottled = true

    setTimeout(() => {
      isThrottled = false
    }, wait)

    func.apply(this, args)
  }
}
```

## 相關題目

1. [bigfrontend.dev - chunk](https://bigfrontend.dev/problem/implement-lodash-chunk)

## 延伸閱讀

1.[Debounce vs Throttle: Definitive Visual Guide](https://kettanaito.com/blog/debounce-vs-throttle)
